...
const Comboboxs = ({
    ...
    useEffect(() => {
        if (dataset)
            setSelected(dataset?.find((e: any) => e.value == defaultValue) ?? null)
    }, [dataset])
    ...
    return (
        <>
            <div className="relative w-full">
                <Combobox disabled={disable} value={selected} onChange={(e: any) => { onChange(e.value), setSelected(e) }}>
                                    ...
                                    filtered?.map((x: any | unknown, i: number) => {
                                        return (
                                            <Combobox.Option
                                                key={i}
                                                value={x}
                                                className={'cursor-pointer rounded-md px-3 py-3 hover:bg-yellow-500'}
                                            >
