...
import { FaBell, FaBuilding, FaChevronDown, FaChevronUp, FaLine, FaUsers } from 'react-icons/fa';
...
const SideBar = ({ element }: { element: JSX.Element }) => {
    ...
    return (
        <div>
            ...
            <aside
                className={`fixed h-full top-0 left-0 z-40 w-64 h-screen transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
                aria-label="Sidebar"
            >
                <div className="h-full px-3 py-4 overflow-y-auto bg-gray-800 flex flex-col justify-between">
                    <div >
                        <div className='flex items-center justify-between'>
                            <div className="flex items-center pl-2.5 mb-5">
                                ...
                                <p className="self-center text-xl font-semibold whitespace-nowrap text-white">Watchdog</p>
                            </div>
                            <p className='sm:hidden mb-5'><IoMdClose onClick={() => setIsOpen(false)} className="w-5 h-5 text-white" /></p>
                        </div>
                                ...
                                menu.map((e) => {
                                    return (
                                        <li key={e.Name}>
                                            <Link
                                                to={e.Path ?? "#"}
                                                className={`flex items-center p-2 text-gray-900 rounded-lg text-white  hover:bg-gray-700 group ${e.Children ? 'cursor-pointer' : ''}`}
                                                onClick={() => e.Children && toggleSubmenu(e.Name)}
                                            >
                                                <div className={`text-2xl flex items-center p-2 text-gray-900 rounded-lg text-white  hover:bg-gray-700 group ${e.Children ? 'cursor-pointer' : ''}`} >
                                                    {e.Icon}
                                                </div>
                                                <span className="ml-3">{e.Name}</span>
                                                {e.Children && (
                                                    <div className="ml-auto">
                                                        {submenuOpen[e.Name] ? <FaChevronUp /> : <FaChevronDown />}
                                                    </div>
                                                )}
                    ...
                    <div className=' w-full flex items-center mb-20 md:mb-0 cursor-pointer text-white' onClick={Logout}>
                        <div className="text-2xl flex items-center p-2 text-gray-900 rounded-lg text-white hover:bg-gray-700 group">
                            <IoMdLogOut />
                        </div>
